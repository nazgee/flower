...
public enum LoadableSeed {
	...
	// Constructors
	// ===========================================================
	private LoadableSeed(final int pID, final int pCost, final int pBlossomID, final int pSeedID) {
		this(pID, pCost, pBlossomID, pSeedID, true);
	}

	private LoadableSeed(final int pID, final int pCost, final int pBlossomID, final int pSeedID, final boolean pLockedByDefault) {
		this(pID, pCost, pBlossomID, pSeedID, pLockedByDefault, Color.WHITE);
	}

	private LoadableSeed(final int pID, final int pCost, final int pBlossomID, final int pSeedID, final boolean pLockedByDefault, final Color ... pColors) {
		id = pID;
		cost = pCost;
		seedID = pSeedID;
	...
	public Color getRandomColor() {
		return col_plant[MathUtils.RANDOM.nextInt(col_plant.length)];
	}

	public static LoadableSeed getSeedById(final int pID) {
		switch (pID) {
		case 1:
			return SEED1;
		case 2:
